...
// #endif
    ...
    QString scalingStr = getNamedArgument(args, "--scaling");
    QVariant cmdScaling;

    if (!scalingStr.isEmpty()) {
        scalingStr.remove('%'); // Strip the '%' character if present
        bool ok;
        float scalingValue = scalingStr.toFloat(&ok);
        if (ok) {
            // Convert to a float multiplier (e.g., 60 becomes 0.6)
            cmdScaling = QVariant(scalingValue / 100.0f);
        }
    }
    // Expose the processed value to the QML frontend
    engine.rootContext()->setContextProperty("cmdScaling", cmdScaling);
